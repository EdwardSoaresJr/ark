...
@section('content')
    ...
    @if (! empty($connectCloudAfterInstall))
        <p class="lead">
            Next, connect ARK Cloud. Sign in with the shop admin account you just created, then continue the Cloud connection.
        </p>
        <div class="actions">
            <a class="btn btn-primary" href="{{ route('login', ['redirect' => route('operations.cloud.connect-after-setup')]) }}">
                Sign in to connect ARK Cloud
            </a>
            <a class="btn btn-secondary" href="{{ route('login') }}">Enter ARK without connecting</a>
        </div>
    @else
        <p class="lead">Optional Cloud services can be connected later in Settings → ARK Cloud.</p>
        <div class="actions">
